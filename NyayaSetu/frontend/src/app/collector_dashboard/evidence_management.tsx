    ...
    <Card className="max-w-3xl w-full">
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><strong>Collector Name:</strong> {selectedEvidence.collectorName}</div>
          <div><strong>Collector Wallet:</strong> {selectedEvidence.collectorWallet}</div>
          <div><strong>Officer Name:</strong> {selectedEvidence.officerName}</div>
          <div><strong>Officer Wallet:</strong> {selectedEvidence.officerWallet}</div>
          <div><strong>Description:</strong> {selectedEvidence.evidenceDescription}</div>
          <div><strong>Type:</strong> {selectedEvidence.evidenceType}</div>
          <div><strong>Condition:</strong> {selectedEvidence.evidenceCondition}</div>
        </div>
        {/* Evidence Fields */}
        {selectedEvidence.evidenceFields && (
          <div className="mt-4">
            <h3 className="font-semibold mb-2">Device Details</h3>
            <ul className="list-disc ml-6">
              {Object.entries(selectedEvidence.evidenceFields).map(([key, value]) => (
            ...
            <ul className="list-disc ml-6">
              {selectedEvidence.files.map((file, idx) => (
                <li key={file._id || idx}>
                  ...
                  <div><strong>File Size:</strong> {file.fileSize}</div>
                  <div><strong>File Type:</strong> {file.fileType}</div>
