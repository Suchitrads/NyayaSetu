...
import { BadgeCheck, FileBarChart, CircleX } from "lucide-react";
        ...
        const fetchCaseSummary = async () => {
            try {
                ...
                const statusCounts = response.data.map(status => ({
                    label: status._id,
                    count: status.count,
                    icon: status._id === "Active" ? <BadgeCheck className="w-5 h-5 text-yellow-500" /> :
                        status._id === "Under Investigation" ? <FileBarChart className="w-5 h-5 text-yellow-500" /> :
                        ...
                        <Card key={index} className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 shadow-sm p-2">
                            <CardHeader className="p-1">
                                <CardTitle className="flex justify-center gap-2 text-xs md:text-sm text-gray-700 dark:text-gray-300">
                                    {status.icon} {status.label} Cases
                                </CardTitle>
                            </CardHeader>
                ...
                <Card className="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 shadow-sm">
                    <CardHeader>
                        <CardTitle className="flex items-center gap-2 text-base md:text-lg text-yellow-600 dark:text-yellow-500">
                            <FileBarChart className="w-5 h-5" /> Recently Accessed Cases
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        {/* ✅ Scrollable wrapper to enable horizontal scrolling on small screens */}
                            ...
                            <Table className="min-w-max table-auto">
                                <TableHeader>
                                    <TableRow className="bg-gray-100 dark:bg-gray-700">
                                        ...
                                        <TableHead className="text-xs md:text-sm whitespace-nowrap">Investigating Officer</TableHead>
                                ...
                                <TableBody>
                                    {cases.map((c) => (
                                        <TableRow key={c.caseId} className="border-b border-gray-200 dark:border-gray-700">
                                            <TableCell className="text-xs md:text-sm whitespace-nowrap">{c.caseId}</TableCell>
                                            <TableCell className="text-xs md:text-sm whitespace-nowrap">{c.caseName}</TableCell>
                                            <TableCell className="text-xs md:text-sm whitespace-nowrap">{c.selectedOfficer}</TableCell>
                                            <TableCell className="text-xs md:text-sm whitespace-nowrap">{new Date(c.updatedAt).toLocaleString()}</TableCell>
                                            <TableCell className="hidden md:table-cell text-xs md:text-sm whitespace-nowrap">{c.description}</TableCell>
                                            <TableCell>
                                                <Badge className={`text-xs md:text-sm px-3 py-1 rounded-full ${c.status === "Active" ? "bg-yellow-500 text-white dark:bg-yellow-600" :
                                                    c.status === "Under Investigation" ? "border border-yellow-500 text-yellow-600 bg-transparent" :
